...
def main():
    ...
    #BNLJ2
    a = 2   #获取数据的起始行数
    c = 3206   #增量合并本年累计营收填报数据的起始行数
    d = 3507   #存量本年累计营收填报数据的起始行数
    for x in range(0,8):
        list2 = [[c+x,7]] #增量合并本年累计营收
        b_row = d+x   #数据坐标行数
        print(b_row)
        b_column = 24   #数据坐标列数
        a_cell_pl_list = tools.excel_cell_bylist(b_file,b_file_sheet,list2)   #获取增量合并本年累计营收
        amount = 0
        for i in range(len(a_cell_pl_list)):
            if a_cell_pl_list[i] == "":
                a_cell_pl_list[i] = 0
    ...
    for x in range(0,8):
        ...
        for i in range(len(a_cell_pl_list1)):
            if a_cell_pl_list1[i] == "":
                a_cell_pl_list1[i] = 0
            elif a_cell_pl_list1[i] == -1e-06:
                a_cell_pl_list1[i] == 0
            elif a_cell_pl_list1[i] == 1e-06:
                a_cell_pl_list1[i] = 0
            if a_cell_pl_list2[i] == "":
                a_cell_pl_list2[i] = 0
            elif a_cell_pl_list2[i] == -1e-06:
                a_cell_pl_list2[i] == 0
            elif a_cell_pl_list2[i] == 1e-06:
                a_cell_pl_list2[i] = 0
